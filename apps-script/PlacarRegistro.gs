/** Registro do placar, avanço, repescagem, 3º lugar e histórico */
function registrarResultado_(jogo,payload){
 ...
 if(proximoJogo){const ni=dados.findIndex(x=>numero_(x[1])===proximoJogo),equipe=equipes.find(e=>e.id===vencedorId);if(ni>=0&&equipe){const col=proximoSlot===1?4:6;s.getRange(ni+2,col,1,2).setValues([[equipe.id,nomeEquipe_(equipe)]])}}
 const fase=texto_(r[2]).toUpperCase();
 if(fase==='FASE INICIAL'){
  const iniciais=dados.map((x,idx)=>({row:x,index:idx})).filter(x=>texto_(x.row[2]).toUpperCase()==='FASE INICIAL').sort((a,b)=>numero_(a.row[1])-numero_(b.row[1]));
  const pos=iniciais.findIndex(x=>numero_(x.row[1])===jogo),repIndex=dados.findIndex(x=>texto_(x[2]).toUpperCase()==='REPESCAGEM'),perdedor=equipes.find(e=>e.id===perdedorId);
  if(repIndex>=0&&pos>=0&&perdedor){const col=pos===0?4:6;s.getRange(repIndex+2,col,1,2).setValues([[perdedor.id,nomeEquipe_(perdedor)]])}
 }
 if(fase==='SEMIFINAL'){
  ...
  const semiIndex=semis.findIndex(x=>numero_(x.row[1])===jogo),terceiroIndex=dados.findIndex(x=>texto_(x[2]).toUpperCase()==='DISPUTA DE 3º LUGAR'),equipePerdedora=equipes.find(e=>e.id===perdedorId);
 ...
 if(finalizada&&typeof atualizarCampeonatoFinalizado_==='function')atualizarCampeonatoFinalizado_();
 ...
 return{message:finalizada?'Placar registrado. Competição encerrada e arquivada.':'Placar registrado. Próxima partida liberada em '+intervalo+' minutos.',state:obterEstadoAdmin_()};
}
...
function atualizarLiberacoes_(){const s=aba_(VOLEI.SHEETS.CHAVEAMENTO),l=s.getLastRow();if(l<2)return;const d=s.getRange(2,1,l-1,24).getValues();if(d.some(r=>['LIBERADO','EM_DISPUTA'].indexOf(texto_(r[16]))>=0))return;const agora=Date.now(),candidatas=[];for(let i=0;i<d.length;i++){const r=d[i],disp=r[18]?interpretarData_(r[18]):null;if(r[3]&&r[5]&&texto_(r[16])==='AGUARDANDO'&&disp&&disp.getTime()<=agora)candidatas.push({i:i,r:r});}candidatas.sort((a,b)=>prioridadePartida_(a.r)-prioridadePartida_(b.r));if(candidatas.length)s.getRange(candidatas[0].i+2,17).setValue('LIBERADO');}
