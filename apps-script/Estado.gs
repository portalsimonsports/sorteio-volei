...
function dataEventoIso_(valor){const s=dataConfigDdMmYyyy_(valor),m=s.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);return m?m[3]+'-'+m[2]+'-'+m[1]:s;}
...
function obterEstadoPublicoSemVerificacao_(){
 ...
 const terceiro=matches.filter(m=>texto_(m.phase).toUpperCase()==='DISPUTA DE 3º LUGAR')[0],agenda={eventDate:dataEventoIso_(c.DATA_EVENTO||'22/07/2026'),registrationCloseTime:horarioConfigHHMM_(c.ENCERRAMENTO_INSCRICOES||'09:50'),countdownStartTime:horarioConfigHHMM_(c.INICIO_CONTAGEM||'09:55'),firstMatchTime:horarioConfigHHMM_(c.PRIMEIRA_PARTIDA_HORARIO||'10:15'),timezone:VOLEI.TIMEZONE},regras=regrasPartida_();
