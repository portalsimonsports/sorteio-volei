...
function criarJogoCompacto_(game,roundIndex,phase){return{game:game,roundIndex:roundIndex,phase:phase,team1:null,team2:null,team1Placeholder:'',team2Placeholder:'',scores:[[null,null],[null,null],[null,null]],sets1:0,sets2:0,winnerId:'',status:'AGUARDANDO',finishedAt:'',availableAt:'',nextGame:0,nextSlot:0};}
...
function montarChaveamento_(equipes,seed){
 ...
 if(quantidadePreliminar>0){
  rodadaPreliminar={index:0,name:'FASE PRELIMINAR',matches:[]};
  for(let i=0;i<quantidadePreliminar;i++){const m=criarJogoCompacto_(game++,0,'FASE PRELIMINAR');m.team1=timesPreliminares[i*2];m.team2=timesPreliminares[i*2+1];rodadaPreliminar.matches.push(m);}
 ...
 if(semis&&semis.matches.length===2&&finalRound){finalRound.name='DECISÕES';finalRound.matches.push({game:game++,roundIndex:finalRound.index,phase:'DISPUTA DE 3º LUGAR',team1:null,team2:null,team1Placeholder:'Perdedor Jogo '+semis.matches[0].game,team2Placeholder:'Perdedor Jogo '+semis.matches[1].game,scores:[[null,null],[null,null],[null,null]],sets1:0,sets2:0,winnerId:'',status:'AGUARDANDO',finishedAt:'',availableAt:'',nextGame:0,nextSlot:0});}
