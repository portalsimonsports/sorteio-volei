...
function montarChaveamento_(equipes){
 const size=proximaPotenciaDois_(equipes.length),total=Math.log(size)/Math.log(2),slots=equipes.slice(),rounds=[];while(slots.length<size)slots.push(null);let game=1;
 ...
 const first=todosJogos_(rounds).filter(m=>m.team1&&m.team2&&m.status==='AGUARDANDO').sort((a,b)=>a.game-b.game)[0];
