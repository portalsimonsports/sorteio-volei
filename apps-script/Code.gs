/** SORTEIO DE DUPLAS DE VÔLEI — V018 */
const VOLEI=Object.freeze({
 ...
 SHEETS:Object.freeze({CONFIG:'CONFIG',JOGADORES:'JOGADORES',EQUIPES:'EQUIPES',CHAVEAMENTO:'CHAVEAMENTO',SORTEIOS:'SORTEIOS',LOG:'LOG',LISTAS:'LISTAS',CAMPEONATOS:'CAMPEONATOS',HISTORICO_EQUIPES:'HISTORICO_EQUIPES',HISTORICO_CHAVEAMENTO:'HISTORICO_CHAVEAMENTO',DESEMPENHO:'DESEMPENHO_PARTICIPANTES'}),
 HEADERS:Object.freeze({
  JOGADORES:['ID','NOME','DATA_NASCIMENTO','IDADE','POTE','CATEGORIA','NOTA_DESEMPENHO','NOTA_AJUSTADA','ATIVO','DATA_CADASTRO','OBSERVAÇÃO','AJUSTE_HISTORICO','INDICE_ATUAL','INDICE_ATUALIZADO_EM'],
...
function doGet(e){return executarApi_(e&&e.parameter||{});}
function doPost(e){let b={};try{if(e&&e.postData&&e.postData.contents)b=JSON.parse(e.postData.contents);}catch(ignore){}return executarApi_(Object.assign({},e&&e.parameter||{},b||{}));}
function executarApi_(p){try{const a=texto_(p.acao||'estado');let d;switch(a){
...
function responder_(o,cb){const j=JSON.stringify(o);if(cb&&/^[A-Za-z_$][0-9A-Za-z_$\.]*$/.test(String(cb)))return ContentService.createTextOutput(String(cb)+'('+j+');').setMimeType(ContentService.MimeType.JAVASCRIPT);return ContentService.createTextOutput(j).setMimeType(ContentService.MimeType.JSON);}
...
function numeroOpcional_(v){if(v===''||v==null)return null;const n=Number(String(v).replace(',','.'));return isFinite(n)?n:null;}
...
function hashCodigo_(c){return hash_((props_().getProperty('ACTIVATION_SALT')||'SALT')+'|'+String(c));}
...
function log_(e,s,o,u,d,n,id){aba_(VOLEI.SHEETS.LOG).appendRow([new Date(),e||'',s||'',o||'',u||'',d||'',n||'INFO',id||'']);}
