(() => {
  ...
  if (scoreForm) scoreForm.noValidate = true;
  ...
  ui.drawNow?.addEventListener('click', async () => {
    ...
    finally { A.busy(ui.drawNow, false); }
  });
  ...
  ui.clearAll?.addEventListener('click', async () => {
    ...
    const message = local
      ...
      : 'Apagar definitivamente participantes, equipes, placares e histórico do sistema?';
    if (!confirm(message)) return;
    try {
      const result = await V.request('limparTudo');
      V.invalidateReadCache?.();
      V.toast(local ? 'Dados locais apagados.' : 'Dados do sistema apagados.');
      if (result?.state) A.render(result.state); else await A.refresh();
    } catch (error) { V.toast(error.message, 'error'); }
  });
  ...
  scoreForm?.addEventListener('submit', async event => {
    ...
    const score = { s1a:value('scoreS1A'), s1b:value('scoreS1B'), s2a:value('scoreS2A'), s2b:value('scoreS2B'), s3a:value('scoreS3A'), s3b:value('scoreS3B') };
    ...
    try {
      ...
      const result = await V.request('registrarResultado', { jogo: scoreGame.value, vencedorId: payload });
      V.invalidateReadCache?.();
      A.clearScoreDraft?.(scoreGame.value);
      scoreForm.reset();
      V.toast(`Placar salvo. Intervalo de ${C.MATCH_INTERVAL_MINUTES || 10} minutos iniciado.`);
      if (result?.state) A.render(result.state); else await A.refresh();
    } catch (error) { V.toast(error.message, 'error'); }
    finally { A.busy(saveScore, false); }
  });

  if (ui.sheetLink) ui.sheetLink.href = C.SHEET_URL || '#';
  A.preview();
  A.refresh();

  // O painel administrativo não precisa consultar a base a cada 10 segundos.
  // Um único ciclo por minuto evita concorrência e mantém a atualização automática.
  setInterval(() => {
    if (!A.scoreIsDirty?.() && document.visibilityState === 'visible') A.refresh();
  }, 60000);
