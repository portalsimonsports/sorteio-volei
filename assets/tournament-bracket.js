(() => {
  ...
  function teamFromMembers(member1, member2, index) {
    ...
    return B.normalizeTeam({
      ...
      member1Id: member1.id, member1: member1.name, member1Pot: member1.pot,
      member1Score: B.num(member1.score), member1Index: B.num(member1.adjustedScore),
      member2Id: member2.id, member2: member2.name, member2Pot: member2.pot,
      member2Score: B.num(member2.score), member2Index: B.num(member2.adjustedScore),
      type,
  ...
  function pairExtremes(list, teams) {
    const remaining = [...list].sort((a, b) => B.num(a.adjustedScore) - B.num(b.adjustedScore) || a.name.localeCompare(b.name, 'pt-BR'));
  ...
  function balanceTeams(players) {
    ...
    const adults = active.filter(player => player.pot === 'A')
      .sort((a, b) => B.num(b.adjustedScore) - B.num(a.adjustedScore) || a.name.localeCompare(b.name, 'pt-BR'));
    const children = active.filter(player => player.pot === 'B')
    ...
    // Adultos excedentes: permanecem os de menor índice.
    pairExtremes(adults.slice(mixedCount), teams);
    // Crianças excedentes: permanecem as de maior índice.
  ...
  function allMatches(rounds) { return rounds.flatMap(round => round.matches); }

  function advanceWinner(rounds, match) {
    if (!match.nextGame || !match.winnerId) return;
    const next = allMatches(rounds).find(item => Number(item.game) === Number(match.nextGame));
    const team = winnerTeam(match);
    if (!next || !team) return;
    if (Number(match.nextSlot) === 1) next.team1 = team; else next.team2 = team;
  }

  function advanceLoserToThird(rounds, match) {
    if (String(match?.phase || '').toUpperCase() !== 'SEMIFINAL') return;
    const thirdPlace = allMatches(rounds).find(item => String(item.phase || '').toUpperCase() === THIRD_PLACE_PHASE);
    const loser = loserTeam(match);
    if (!thirdPlace || !loser) return;
    const semifinals = allMatches(rounds).filter(item => String(item.phase || '').toUpperCase() === 'SEMIFINAL')
  ...
  function createInitialSlots(teams, size, seed) {
    ...
    pairingTypes.forEach((type, matchIndex) => {
      if (type === 'BYE') {
        ...
        const left = hash32(`${seed}|LADO_BYE|${matchIndex}`) % 2 === 0;
        slots.push(left ? team : null, left ? null : team);
      } else {
  ...
  function buildBracket(teams, seed) {
    ...
    for (let roundIndex = 0; roundIndex < totalRounds; roundIndex++) {
      ...
      for (let index = 0; index < matchCount; index++) {
        matches.push(B.normalizeMatch({
          game: game++, roundIndex, phase: phaseName(size, roundIndex, totalRounds),
          team1: null, team2: null, team1Placeholder: '', team2Placeholder: '',
          winnerId: '', status: 'AGUARDANDO', nextGame: 0, nextSlot: 0
        }));
      }
    ...
    rounds[0].matches.forEach((match, index) => {
      ...
      if (match.team1 && !match.team2) { match.winnerId = match.team1.id; match.status = 'BYE'; }
      else if (!match.team1 && match.team2) { match.winnerId = match.team2.id; match.status = 'BYE'; }
    });
    ...
    if (shuffled.length >= 4 && totalRounds >= 2) {
      ...
      finalRound.matches.push(B.normalizeMatch({
        game: game++, roundIndex: totalRounds - 1, phase: THIRD_PLACE_PHASE,
    ...
    const firstPlayable = allMatches(rounds).find(match => match.team1 && match.team2 && match.status === 'AGUARDANDO');
    if (firstPlayable) {
  ...
  function runDraw() {
    ...
    state.auditHash = hash32(JSON.stringify({ teams, rounds: state.rounds, seed })).toString(16).toUpperCase();
    B.saveState(state);
    return { message: 'Sorteio realizado.', state };
  }

  window.VoleiTournament = {
    THIRD_PLACE_PHASE, balanceTeams, hash32, winnerTeam, loserTeam,
    advanceWinner, advanceLoserToThird, buildBracket, runDraw
  };
})();
